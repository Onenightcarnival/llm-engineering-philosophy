...
这个物理约束意味着：Schema 的字段排列不仅是数据结构的组织方式，它同时定义了 LLM 的推理路径。
...
这就是 Schema as Workflow 的含义：字段排列隐式地定义了 Chain-of-Thought 的步骤。传统的 CoT 通过自然语言指令引导推理（"先想一想...然后分析..."），声明式 CoT 通过结构定义引导推理。前者依赖 LLM 对自然语言指令的理解，后者通过输出格式的物理约束来确定推理的顺序和分步。

需要明确一个边界：结构约束的是推理的**顺序和分步**，不是推理的**质量**。LLM 完全可以按字段顺序生成但在某个字段内敷衍了事。声明式 CoT 保证了"先做 A 再做 B"的执行顺序，但"A 做得好不好"仍然取决于 Field description 的质量和模型本身的能力。
...
字段之间的依赖关系需要在 description 中显式声明。LLM 在生成后续字段时虽然能"看到"前序字段的值，但不意味着它会主动利用这些值——除非 description 明确告诉它这么做。
...
这个差异在简单结构中看似微小，但在复杂的多字段结构中，隐式依赖会导致不可预测的行为。一个有 10 个字段的模型，如果字段之间的依赖关系没有在 description 中显式声明，LLM 可能以任意方式处理字段之间的关系——有时碰巧正确，有时完全偏离预期。

显式依赖声明的格式是直接的：在后续字段的 description 中引用前序字段的名称，说明它们之间的关系。"基于上述 X"、"综合 X 和 Y"、"在 X 的基础上"——这些描述建立了推理步骤之间的逻辑连接。
...
**依赖在前，被依赖在后。** 如果字段 B 的 description 引用了字段 A，A 必须在 B 之前定义。这是逻辑依赖的物理映射——A 的值在 B 生成时已经存在，B 的推理可以真实地建立在 A 的结果之上。
