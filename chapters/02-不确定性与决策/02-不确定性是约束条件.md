...
概率性输出之于 LLM，正如波动性之于市场。它是自回归生成机制的本质产物，无法被"修复"。[第一章](../01-认识论/autoregressive-generation.md)已经从数学层面证明了这一点：每一步生成都是条件概率分布上的采样，概率性是写在机制里的，不在机制外面。

工程师面对概率性输出的本能反应通常是"怎么消除它"。这个反应和早期金融学者面对波动性的反应一样，方向就是错的。正确的问题是"在给定的不确定性水平下，怎么设计出可靠的系统"。
...
**管理范式假设不确定性是本质的。** 在这个范式下，系统需要在不确定性存在的条件下正常工作。工程努力的方向是设计能容忍不确定性的架构：验证层、降级策略、冗余机制、人工回退——目标是让系统在 LLM 输出不完美的情况下依然可靠。
...
在计算数学中，约束条件是问题定义的一部分。没有约束的优化问题要么无解，要么解是平凡的。约束的存在使得问题有了结构，结构使得问题可解。

不确定性可以被形式化为两个核心约束参数：**单次调用可靠性**（即使是最好的 prompt + 最好的模型，单次调用也有失败概率）和**输出变异性**（同一输入的多次调用，输出的一致性程度）。这两个参数不是"坏事"——它们是问题空间的精确描述。好的架构是在这些约束下的最优解，而不是无视这些约束的理想化方案。

两个关键公式：

**串联可靠性。** n 步串联的系统可靠性 = p^n。每增加一步，可靠性都在乘以一个小于 1 的数。

**冗余需求。** 要达到目标可靠性 T，需要的独立调用次数 n = log(1-T) / log(1-p)。其原理是：n 次独立调用中至少一次成功的概率 = 1 - (1-p)^n。

用两个典型场景来展示这些约束的量化含义：

| 指标 | 分类任务（p=0.92） | 生成任务（p=0.78） |
|------|-------------------|-------------------|
| 3 步串联可靠性 | 77.9% | 47.4% |
| 达到 99% 可靠性需要的冗余调用 | 3 次 | 16 次 |

这个分析揭示了一个关键洞察：冗余是管理不确定性的基本工具。如果单次调用的可靠性是 92%，三次独立调用取最佳结果的可靠性是 1 - (1-0.92)^3 = 99.95%。这个计算告诉架构师：与其追求单次调用的完美（从 92% 提升到 99% 可能需要重写整个 prompt 体系），不如通过冗余调用来达到同样的可靠性目标（成本增加 3 倍，但架构简单、可预测）。
...
类比财务预算：总预算是固定的，分配到各个环节的预算之和不能超过总预算。分配公式来自串联可靠性：如果系统能容忍 5% 的失败率，而系统有 n 个 LLM 调用环节，那么每个环节分配到的失败率预算为：

每步失败率 = 1 - (1 - 总失败率)^(1/n)

以一个 3 步 LLM 流水线为例，系统容忍 5% 失败率：每步失败率预算 = 1 - 0.95^(1/3) = 1.70%，即每步可靠性需达到 98.3%。

假设实际各步可靠性为 95%、98%、97%，系统可靠性 = 0.95 x 0.98 x 0.97 = 90.3%，实际失败率 9.7%，超出 5% 的预算。此时战略选项是：

1. 减少步骤数（Strategy > Analysis）
2. 为关键步骤增加冗余
3. 降低业务对可靠性的要求（重新谈判预算）

而**不是**：把每一步的 prompt 优化到极致。
...
**降级策略模式。** 当 LLM 输出不可用时，系统降级到一个质量较低但确定性的方案。
...
管理不确定性不是免费的。每一种管理手段都有成本，正确的工程决策需要在可靠性和成本之间找到平衡。工程决策是在给定预算下最大化可靠性，或在给定可靠性目标下最小化成本。

假设基础参数：单次调用可靠性 92%，单次成本 $0.01，单次延迟 500ms。

**重试策略**的可靠性公式：有效可靠性 = 1 - (1-p)^n，其中 n 为最大重试次数。延迟在最坏情况下串行累加。

| 最大重试次数 | 有效可靠性 | 期望成本 | 最坏延迟 |
|-------------|-----------|---------|---------|
| 1 | 92.00% | $0.01 | 500ms |
| 2 | 99.36% | $0.0108 | 1,000ms |
| 3 | 99.95% | $0.0109 | 1,500ms |
| 5 | 99.9997% | $0.0109 | 2,500ms |

**多数投票策略**的可靠性公式：P(至少 ceil(n/2) 次正确) = sum C(n,k) x p^k x (1-p)^(n-k)。成本固定为 n 倍，但如果并行执行，延迟不增加。
...
这个对比揭示了一个重要的决策支点：重试策略在延迟上的代价是串行累加的（最坏情况下延迟翻倍），但成本的期望值增加有限（因为大多数时候第一次就成功了）。多数投票策略在成本上是固定的倍增，但如果可以并行执行，延迟不增加。选择哪种策略取决于系统的核心约束是成本还是延迟。
