...
"AI Agent"这个术语被赋予了过多的想象空间。自主决策、目标驱动、环境感知、持续学习——这些描述让 Agent 听起来像是人工通用智能的前奏。但在当前的工程实践中，一个 LLM Agent 的结构可以被精确分解为三个组件的组合：
...
**工具应该是幂等的或至少是安全的。** Agent 的循环特性意味着同一个工具可能被多次调用。如果工具有副作用（发送邮件、创建订单、删除数据），重复调用可能造成问题。幂等设计（重复调用产生相同结果）是理想选择；如果无法幂等，至少应该在工具层面实现去重或确认机制。
...
Agent 的循环控制引入了不确定性的二次方——LLM 的每次决策都是概率性的，多次决策的组合让系统行为更加难以预测。在以下场景中，更简单的架构是更好的选择：
...
这些判断标准回到了[第二章](../02-不确定性与决策/00-概述.md)的核心原则：战略决策先于战术实现。选择 Agent 架构还是简单 pipeline，是一个架构级别的战略决策，应该在编写任何代码之前做出。
