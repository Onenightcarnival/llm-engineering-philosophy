...
本文是一篇观点文章，不是技术分析。它表达的是一个经过审慎思考的工程判断，而非一个可量化证明的技术结论。
...
用框架特有的术语重新命名这些操作，制造了一种"LLM 编程需要特殊工具"的错觉。实际上，Python 的函数、列表、字典和标准库已经提供了这些能力。框架做的不是提供新能力，而是将已有能力包装在一个统一的接口下。
...
当流程是"先检索、再生成"时，一个函数调用另一个函数就够了。当流程是"根据意图路由到不同的处理器"时，一个 if-else 就够了。为这些简单的流程引入 Chain 的抽象和 LCEL（LangChain Expression Language）的语法，不是简化，是复杂化。

真正需要复杂编排的场景——分布式执行、持久化状态机、可视化工作流——应该使用为这些场景设计的通用工具（Temporal、Prefect、Airflow），而非 LLM 领域特有的框架。通用编排工具经过了更广泛的生产验证，有更活跃的社区和更稳定的 API。
...
**只引入你需要的组件。** 如果你只需要向量检索，引入一个向量数据库客户端（而非整个 RAG 框架）。如果你只需要文本切分，写一个 50 行的切分函数（而非引入一个 Loader-Splitter 框架）。
