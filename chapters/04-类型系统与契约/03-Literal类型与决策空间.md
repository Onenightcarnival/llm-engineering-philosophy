...
这就是为什么 `general_inquiry` 这个兜底选项在实践中几乎是必须的。它不是设计上的懒惰，而是对决策空间完备性的工程保障——承认存在当前枚举无法覆盖的情况，并为这些情况提供显式的处理路径。
...
这个权衡的数学结构与信息论中的量化问题同构。量化器将连续信号映射到有限的离散级别：级别太少导致失真大（信息损失），级别太多导致量化噪声相对增大（分辨率超过了信号的固有精度）。LLM 的分类也遵循相同的规律：枚举粒度应该匹配 LLM 在该任务上的实际区分能力，而不是业务逻辑的理想精度。
