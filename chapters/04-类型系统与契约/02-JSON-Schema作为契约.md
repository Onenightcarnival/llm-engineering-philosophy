...
这个有损性不是缺陷，而是声明式和命令式的天然边界。JSON Schema 能表达的，交给 Schema 验证器；JSON Schema 不能表达的，交给 Pydantic 的运行时验证。两层防线各司其职。
...
这些关系性约束在实际业务中普遍存在，但只能在 Pydantic 的 validator 层面处理，或者通过 prompt 中的自然语言说明来"提示" LLM。这就是为什么本章需要四篇文章来完整讨论类型系统与契约——JSON Schema 是契约的结构骨架，Literal 类型是决策空间的显式化，validator 是运行时不变量的最后防线。它们不是替代关系，而是互补关系。
